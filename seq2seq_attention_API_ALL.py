...
class seq2seq_Attention_all():
    ...
    def __init__(self):
        self.path_to_file = file_path + '/QA_all_char.txt'
        ...
        self.BATCH_SIZE = 16  # small data 64 big 32
        self.embedding_dim = 200 #small data 100 big 200
        self.units = 512 #small data 128 big 512
...
# Download the file
    ...
    def train(self):
        ...
        attenc_lstm = LSTM(units=self.units, return_sequences=True, return_state=True, name = 'encoder_attention_lstm')
        ...
        attdec_lstm = LSTMWithAttention(units=self.units, return_sequences=True, return_state=True, name = 'decoder_attention_lstm')
        ...
        attdec_d1 = Dense(self.units, activation="relu", name = 'Dense1')
        attdec_d2 = Dense(self.vocab_out_size, activation="softmax", name = 'Dense2')
        attdec_out = attdec_d2(Dropout(rate=.4, name='Dropout2')(attdec_d1(Dropout(rate=.4, name='Dropout1')(attdec_lstm_out))))
...
#                         validation_split=0.2)
        ...
        attmodel.fit_generator(self.generate_batch_data_random(train_data, teach_data, train_data_y, self.BATCH_SIZE),
...
#        
    ...
    def createAttentionInference(self, model, attention_mode=False):
        ...
        model.load_weights(file_path + "/P3.h5", by_name = True)
        ...
        decoder_state_h = Input(shape=(self.units,))
        decoder_state_c = Input(shape=(self.units,))
        ...
        dropout1 = Attention_model.get_layer('Dropout1')(decoder_out)
        
        dense1 = Attention_model.get_layer('Dense1')(dropout1)
        
        dropout2 = Attention_model.get_layer('Dropout2')(dense1)
        
        dense2 = Attention_model.get_layer('Dense2')(dropout2)
        
        decoder_model = Model(inputs=[attention_decoder_inputs, decoder_state_h, decoder_state_c,  attenc_seq_out], 
                              outputs=[dense2 , decoder_h, decoder_c])
...
test = seq2seq_Attention_all()
#test.train()
model = test.create_model()
encoder_Inference, decoder_Inference = test.createAttentionInference(model)
print(test.translate('小烏來線資訊：小烏來線於103年4月6日起，假日都提供來回。', encoder_Inference, decoder_Inference, True))
#
